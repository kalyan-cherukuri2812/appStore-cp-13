...
class AppStore extends Component {
  state = {tabItemStatus: tabsList[0].tabId, searchInput: ''}

  tabStatus = tabId => {
    this.setState({tabItemStatus: tabId})
  }
  ...
  render() {
    const {tabItemStatus, searchInput} = this.state
    const filteredList = appsList.filter(
      each => tabItemStatus === each.category,
    )
    const searchresult = filteredList.filter(each =>
        ...
        <ul className="tab-ul">
          {tabsList.map(each => (
            <TabItem
              ...
              isActive={tabItemStatus === each.tabId}
            />
          ))}
        </ul>
        <ul className="app-ul">
          {searchresult.map(each => (
            <AppItem appListDetails={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
